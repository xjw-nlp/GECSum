...
from modeling_bart_ours import BartScorer
from modeling_pegasus_ours import PegasusScorer
...
class GECSum(nn.Module):
    ...
    def forward(self, text_id, candidate_id, normalize=True, score_mode="base", length_penalty=1, require_gold=True, adding=0, reverse=False):
        if reverse:
            batch_size = candidate_id.size(0)
            num_seqs = len(text_id) // batch_size
            tgt_seq_len = candidate_id.shape[-1]
            candidate_id = torch.repeat_interleave(candidate_id, num_seqs, dim=0).contiguous().view(batch_size, -1, tgt_seq_len)

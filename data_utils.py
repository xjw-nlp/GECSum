...
class GECSumDataset(Dataset):
    def __init__(self, data_type='train', arrow_obj_path=None, obj_args=None):
        """ data format: article, abstract, [(candidiate_i, score_i)] """
        self.obj_args = obj_args
        self.is_pegasus = self.obj_args.is_pegasus
        self.data_type = data_type
        self.arrow_obj_path = arrow_obj_path
        if data_type == 'train':
            if self.arrow_obj_path:
                self.arrow_data = load_from_disk(self.arrow_obj_path)[data_type]
            else:
...
def build_candidate(args, model, tokenizer, raw_data, epoch_idx, gpuid, data_type='train', bsz=4, num_samples=3600):
    if len(args.gpuid) > 1:
        _model = model.module
    else:
        _model = model
    ...
    for idx in tqdm(sample_ids):
        ...
        if args.config == 'cnndm':
            src_texts.append(data_point['article'])
            tgt_texts.append(data_point['highlights'])
        if count % 100 == 0:
        ...
        if count % bsz == 0:
            ...
            for i, hypothesis in enumerate(dec):
                ...
                dec[i] = hypothesis
            _model.scoring_mode()
            encoder_input_ids = tokenizer.batch_encode_plus(dec, max_length=1024, return_tensors="pt", pad_to_max_length=False, truncation=True, padding=True)["input_ids"].to(device)
            decoder_input_ids = tokenizer.batch_encode_plus(tgt_texts, max_length=1024, return_tensors="pt", pad_to_max_length=False, truncation=True, padding=True)["input_ids"].to(device)
    ...
    if src_texts:
        ...
        for batch_idx, scores in enumerate(output['score']):
            ...
            res_cand_scores.append(ordered_scores)
            cur_pos += num_cands
    dataset = Dataset.from_dict({'src_text': res_src_texts, 'ref_text': res_ref_texts, 'cand_texts': res_cand_texts, 'cand_scores': res_cand_scores})
